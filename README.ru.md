...
| `POST` | `/api/models/:id/benchmark` | Запуск автоматического замера скорости токенов |
...
- `list_models`: Возвращает список сконфигурированных моделей и их статус.
- `start_model`: Запускает модель с указанным профилем.
- `stop_all_models`: Мгновенно завершает все процессы инференса.
- `get_system_status`: Возвращает активную модель, TPS, метрики VRAM / RAM / CPU.
- `benchmark_model`: Запускает замер производительности модели.
